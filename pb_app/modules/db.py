...
class DB:
    ...
    def get_cors(self):
        self.app_cors = self.db_main["sys-apps-setting"].find_one({"appid":config.appid})["cors"]
        
    def get_routes(self):
        # self.app_route = self.db_main["sys-apps-route"].find({"appid":config.appid})
        return self.db_main["sys-apps-route"].aggregate([{"$match":{"appid":config.appid}},{"$sort":{"depth":-1}}])
    ...
    def get_func(self,funcid:str):
        return self.db_main["sys-apps-function"].find_one({"appid":config.appid,"functionid":funcid})